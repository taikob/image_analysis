...
def detect_rotmo(o,op,vec,rd=None):#dimension of o and op must be 2,o: origin, op: position from origin, rd: radius of circle
...
def lucas_kanade(img1, img2, cnt=None, nvec=None, irt=None, rd=None):
    window_size= 50
    quality_level= 0.3
    ...
    try:
        ...
        for i, (new, old) in enumerate(zip(good_new, good_old)):
            ...
            c, d = old.ravel()
            dx = a - c; dy = b - d
            if    cnt: val = detect_rotmo(cnt,[c,d],[dx,dy],rd=rd)
...
def farneback(img1, img2, cnt=None, nvec=None, irt=None, rd=None):
    window_size= 10
    stride= 5
    min_vec= 0.01
    try:
        ...
        for x, y in itertools.product(range(0, width, stride),
            ...
            if np.linalg.norm(flow[y, x]) >= min_vec:
                ...
                if    cnt: val = detect_rotmo(cnt,[x,y],[dx,dy],rd=rd)
                elif nvec: val = unit_vec(nvec, [dx,dy], irt=irt)
                else:      val = np.sqrt(dx**2+dy**2)
...
def get_optical_flow(root,file1, file2, met='lk',cc='yellow',lc='red',vs=None,s=1,l=2, cnt=False, nvec=False, irt=False, rd=False):
    ...
    if met=='lk':
        data, ofabs = lucas_kanade(img1_gray, img2_gray, cnt, nvec,irt,rd)
        if vs is None: vs=50
    elif met=='fb':
        data, ofabs = farneback(img1_gray, img2_gray, cnt, nvec,irt,rd)
        if vs is None: vs=4
